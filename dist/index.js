...
const inhgToKpa = (inHg) => inHg * 3.386389;
const kpaToInhg = (kpa) => kpa / 3.386389;
const kphToMps = (kph) => kph / 3.6;
const mpsToKts = (mps) => mps * 1.9438445;
const ktsToMps = (kts) => kts / 1.9438445;
const metarParser = (metarString) => {
    ...
    const calcHumidity = (temp, dew) => {
        return (Math.exp((17.625 * dew) / (243.04 + dew)) /
            Math.exp((17.625 * temp) / (243.04 + temp)));
    };
    const round = (value, toNext = 500) => {
        ...
        switch (mode) {
            ...
            case 2:
                ...
                if (match) {
                    ...
                    const speedMps = windUnit === 'KPH'
                        ? kphToMps(windSpeed)
                        : windUnit === 'MPS'
                            ? windSpeed
                            : ktsToMps(windSpeed);
                    const gustMps = windUnit === 'KPH'
                        ? kphToMps(windGust)
                        : windUnit === 'MPS'
                            ? windGust
                            : ktsToMps(windGust);
            ...
            case 7:
                ...
                if (match) {
                    ...
                    const pressure = match[1] === 'Q' ? pressureValue / 10 : pressureValue / 100;
    ...
    // Finishing touches
    metarObject.visibility.miles = String(round(Number(metarObject.visibility.miles), 0.5));
    metarObject.visibility.meters = String(round(Number(metarObject.visibility.meters)));
    if (metarObject.clouds) {
